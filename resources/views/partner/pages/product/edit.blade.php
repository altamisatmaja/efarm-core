...
<div class="ransition-colors duration-300">
    <div class="container mx-auto p-4">
        <div class="bg-white shadow rounded-lg p-6">
            <h1 class="text-xl font-semibold mb-4 text-gray-900">Edit product</h1>
            <p class="text-gray-600 mb-6">Tambah aja bos. gak usah tanya</p>
            <form class="form-tambah-product-partner">
                <div class="mb-4">
                ...
                <div class="mb-4 hidden">
                    <label for="id_partner" class="text-sm font-medium text-gray-700 block mb-2">Id Partner
                        readonly</label>
                    <input type="number" value="{{ $partner->id }}" name="id_partner" id="id_partner" placeholder="{{ $partner->id }}"
                        class="border p-2 rounded w-full">
                </div>
...
@push('js')
    <script>
        $(function() {
            // Tambah data
            $('.form-tambah-product-partner').submit(function(e) {
                e.preventDefault();
                const form = $(this);
                ...
                $.ajax({
                    url: '/api/partner/{{ $partner->username }}/product/add',
                    type: 'POST',
                    data: formData,
                    cache: false,
                    contentType: false,
                    ...
                    success: function(data) {
                        if (data.success) {
                            ...
                            window.location.href = "/partner/{{ $partner->username }}/product";
                        }
                    }
                })
            });
