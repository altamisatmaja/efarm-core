...
@section('title', 'Customer | Selesai')
        ...
        @foreach ($allorders as $allorder)
            ...
            <div class="mt-6 sm:mt-8 lg:flex lg:gap-8">
                <div
                    ...
                    <div class="space-y-4 p-6">
                        ...
                        <div class="flex items-center justify-between gap-4">
                            ...
                            <p class="text-sm font-normal text-gray-500 "><span
                                    ...
                                    class="font-medium text-textbase ">Dijual oleh </span>{{ $partner->nama_perusahaan_partner }}</p>
                                    @endforeach
                    ...
                    <div class="flex items-center justify-between gap-4">
                        <p class="text-sm font-normal text-gray-500 "><span
                                class="font-medium text-textbase ">Pengiriman dilakukan dengan </span>{{ $allorder['order']['pengiriman'] }}
                        </p>
                    </div>
                    <div class="flex items-center justify-between gap-4">
                        @foreach ($orderdetails['partner'] as $partner)
                        <p class="text-sm font-normal text-gray-500 "><span class="font-medium text-textbase ">Dikirim dari </span>{{ $partner->provinsi_partner }}, {{ $partner->kabupaten_partner }}, {{ $partner->kecamatan_partner }}, {{ $partner->kelurahan_partner }}, {{ $partner->alamat_partner }}</p>
                        @endforeach
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-6 rounded-lg border border-gray-200 bg-white p-6 shadow-sm  ">
                    ...
                    <ol class="relative ms-3 border-gray-200 ">
                        <li class="mb-10 ms-6">
                            ...
                            <h4 class="mb-0.5 text-base font-semibold text-textbase ">Pesanan {{ $allorder['order']['status'] }}</h4>
                            <p class="text-sm font-normal text-gray-500 ">Nama perusahaan partner</p>
                        </li>
                    </ol>

                    <div class="gap-4 sm:flex sm:items-center">
                        <a href="{{ route('customer.review', $allorder['slug_product']) }}"
                            class="w-full rounded-lg items-center flex justify-center  border border-gray-200 bg-white px-5  py-2.5 text-sm font-medium text-textbase hover:bg-primarybase hover:text-white focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100">Review</a>
                        <a href="{{ route('testimonial.show', $allorder['slug_product']) }}"
                            class="w-full rounded-lg items-center flex justify-center  border border-gray-200 bg-white px-5  py-2.5 text-sm font-medium text-textbase hover:bg-primarybase hover:text-white focus:z-10 focus:outline-none focus:ring-4 focus:ring-gray-100">Ulas</a>
                    </div>
                </div>
            </div>
    </div>
